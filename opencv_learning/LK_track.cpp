...
public:
   ...
   //设置图像处理函数
    ...
    void displayInput(string wn){
        WindowNameInput = wn;
        namedWindow (WindowNameInput,CV_WINDOW_NORMAL);
    }
    //设置输出视频播放窗口
    void displayOutput(string wn){
        WindowNameOutput = wn;
        namedWindow (WindowNameOutput,CV_WINDOW_NORMAL);
    }
    //销毁窗口
    void dontDisplay(){
...
int main(int argc, char *argv[])
{
    ...
    //打开输入视频
    processor.setInput ("road.avi");
    processor.displayInput ("Current Frame");
    processor.displayOutput ("Output Frame");
    //设置每一帧的延时
    processor.setDelay (1000./processor.getFrameRate ());//延时为毫秒
    //设置帧处理函数，可以任意
